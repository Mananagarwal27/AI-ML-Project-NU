...
def set_brightness_percent(percent: int):
        ...
        try:
                ...
                val = percent / 100.0
                subprocess.run(["brightness", str(val)], check=False)
        except Exception:
                pass
...
def fingers_up(lm_list):
        ...
        if not lm_list:
                return [False]*5
        # Thumb: compare tip and ip in x for hand orientation
        # Use landmark indices: tips = [4, 8, 12, 16, 20]; pip-like: [2, 6, 10, 14, 18]
        tip_ids = [4, 8, 12, 16, 20]
        pip_ids = [2, 6, 10, 14, 18]
        # Determine hand label (left/right) by comparing x of wrist and index_mcp maybe
        # Simpler: use relative x of thumb tip vs thumb ip
        thumb_tip_x = lm_list[4][1]
        thumb_ip_x = lm_list[3][1]
        # If thumb tip x < ip x => thumb is to the left -> depends on handedness; we just compare vertical for other fingers
        # For thumb, use x comparison:
        fingers.append(thumb_tip_x < thumb_ip_x)
        # Other fingers: tip y < pip y -> finger up (note: y increases downward)
                ...
                while True:
                        ...
                        if results.multi_hand_landmarks:
                                ...
                                # --- Virtual mouse: index up, others down (control mouse by index tip position) ---
                                        ...
                                        if all(fingers):
                                                now = time.time()
                                                if now - last_play_pause_time > 1.0:
                                                        # Check if hand is fairly still comparing index x to previous
                                                        ix = index_tip[1]
                                                        if prev_index_x is not None and abs(ix - prev_index_x) < 10:
                                                                media_play_pause()
